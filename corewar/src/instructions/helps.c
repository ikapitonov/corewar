...
int		check_arg_types(char *types, char command)
{
	...
	{
		if (!types[count] || (types[count] == T_REG_CODE && !(T_REG & g_instr[(int)command].args[count])))
			return (1);
		if (!types[count] || (types[count] == T_DIR_CODE && !(T_DIR & g_instr[(int)command].args[count])))
			return (1);
		if (!types[count] || (types[count] == T_IND_CODE && !(T_IND & g_instr[(int)command].args[count])))
			return (1);
		++count;
	}
	return (0);
